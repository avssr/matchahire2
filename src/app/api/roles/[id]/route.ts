...
// Sample data for development purposes (same as in roles/route.ts)
...
export async function GET(
  ...
  try {
    ...
    const role = sampleRoles.find(role => role.id === id);
    ...
    logger.error(`Error fetching role with ID: ${params.id}`, error);
    return NextResponse.json(
